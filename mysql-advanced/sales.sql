...
--
-- Host: 127.0.0.1:3306
-- Gegenereerd op: 09 nov 2022 om 17:09
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_create_customer_reporting` (INOUT `pCustomerNumber` INT(100))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_create_emaillist` (INOUT `emailList` VARCHAR(4000))   BEGIN
    DECLARE finished INTEGER DEFAULT 0;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_get_customers` ()   BEGIN
    SELECT 
        customerName, 
        city, 
        state, 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_get_customer_shipping` (IN `pCustomerNUmber` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_get_office_by_country` (IN `countryName` VARCHAR(255))   BEGIN
select *
from offices
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_get_ordercount_by_status` (IN `orderStatus` VARCHAR(25), OUT `total` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_get_total_order` ()   BEGIN
...
--
-- Tabelstructuur voor tabel `customer_reporting`
--
...
--
-- Gegevens worden geëxporteerd voor tabel `customer_reporting`
--

INSERT INTO `customer_reporting` (`customerNumber`, `customerName`, `totalOrderAmount`) VALUES
...
('4', 'Paris', 'OOPS', '43 Rue Jouffroy D\'abbans', NULL, NULL, 'France', '75017', 'EMEA'),
...
--
-- Indexen voor tabel `customer_reporting`
--
ALTER TABLE `customer_reporting`
  ADD PRIMARY KEY (`customerNumber`);
...
--
-- Beperkingen voor tabel `customer_reporting`
--
ALTER TABLE `customer_reporting`
  ADD CONSTRAINT `customer_reporting_ibfk_1` FOREIGN KEY (`customerNumber`) REFERENCES `customers` (`customerNumber`);
